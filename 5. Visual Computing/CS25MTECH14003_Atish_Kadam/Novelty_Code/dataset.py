...
## Change the data partition universally
...
def separate_data(data, data_test, num_clients, num_classes, niid=False, balance=False, partition=None, alpha = 0.1,least_samples=1,class_per_client = 10):
    ...
    if partition == 'pat':
        ...
        try_cnt = 1
        max_tries = 1000  # guard against infinite loop (e.g. CIFAR-10 + many clients + low alpha)
        while min_size < least_samples:
            ...
            if try_cnt > max_tries:
                print(
                    f'WARNING: Could not satisfy least_samples={least_samples} after {max_tries} tries. '
                    f'Proceeding with current partition (min_size={min_size}). '
                    f'Consider lowering --shred, raising --alpha, or using --balance.')
                break
            ...
            # min_size must be evaluated AFTER all K classes are distributed,
            # not inside the inner loop. Previously, min_size was checked after class 0
            # only, where many clients had 0 samples, causing an infinite retry loop.
